...
| Path | Role |
| --- | --- |
| `src/index.ts` | Host half: `GET|POST /mcp-toggle/api` — iterates `ctx.loader.entries()` filtering `@deepseek-ai/dsh-mcp-client`, projects `serverName` from `options.config`, `entry.update({disabled})` for the live toggle, **upserts** a `- id: <rawId>` / `disabled: <bool>` row into `$DSH_HOME/cordis.patch.yml` for persistence. |
...
4. **Persist to the HOME patch layer, not the profile's**: **upsert** `- id: <rawId>` / `disabled: <bool>` into `$DSH_HOME/cordis.patch.yml` (resolve via `process.env.DSH_HOME || ~/.dsh`). The web profile applies bundle → profile `cordis.patch.yml` → HOME → overlays, so home outranks profile and MCP rows from either file are covered. The upsert replaces the existing top-level row's `disabled:` value in place ("last row wins", one row per id — toggles never duplicate rows) and appends only when the id has no top-level row yet; it tolerates CRLF and LF line endings (strip a trailing `\r` for matching, preserve it on write). NEVER rewrite the file wholesale, never touch `cordis.yml` or bundle patches.
...
- **After restart (hard-refresh the tab)**: Settings shows the "MCP Servers" page; `GET /mcp-toggle/api` returns the MCP entry list; toggling a server updates its phase immediately and **upserts** its row in `$DSH_HOME/cordis.patch.yml` (in place — repeated toggles keep exactly one row per id); a further DSH restart keeps the state.
...
- Distinguishing traits: packaged profile plugin (persists across restarts), additive Settings page that never shadows shipped UI, live `Entry.update({disabled})` toggle of MCP client fibers (tools unregister/register), HOME-patch-layer **upsert** persistence (one row per id, never duplicated; outranks profile layers), host HTTP route instead of dynamic RPC, real-Node host half, pure `--dsw-alias-*` theming.
